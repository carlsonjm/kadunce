...
int main()
{
    ...
    require(Kadunce::validBentoProjectionShape(shape),
        "Valid pane/overflow projection shape was rejected");

    auto invalid = shape;
    invalid.rects.pop_back();
    require(!Kadunce::validBentoProjectionShape(invalid),
        "Mismatched pane geometry was accepted");
    invalid = shape; invalid.overflow.push_back(2);
    require(!Kadunce::validBentoProjectionShape(invalid),
        "Duplicate pane/overflow identity was accepted");
    invalid = shape; invalid.lead = 5;
    require(!Kadunce::validBentoProjectionShape(invalid),
        "Overflow member was accepted as visible lead");
