...
The reasoning, measurements and history behind `ROADMAP-CC.md`, kept under the
same block headings. `ROADMAP-CC.md` is the checklist and says what to do; this
says why, what was measured, and what a task needs to know before it starts.
Read the section for the block you are working on, not the whole file.

When a task finishes, tick it in `ROADMAP-CC.md` and put anything worth keeping
--- a measurement, a rejected approach, a physical verdict --- here under its
block. Durable decisions still go to `DECISIONS.md`.
