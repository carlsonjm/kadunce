...
case ${KADUNCE_PROBE_SESSION:-session.sh} in
    ...
    x11-client-runtime-session.sh|x11-baseline-runtime-session.sh|x11-action-runtime-session.sh) ;;
...
timeout 40s env XDG_RUNTIME_DIR="$unload_root/runtime" \
    ...
    dbus-run-session -- "${xwayland_launcher[@]}" kwin_wayland --virtual --width 1280 --height 800 --output-count "$output_count" \
